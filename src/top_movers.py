...
def get_movers(qty, move):
        ...
        for item_old in change_previous:
            ...
            if item_new[0] == item_old[0]:
                if item_new[0] not in top_movers:
                    ...
                    top_movers[item_new[0]] = top_movers[item_new[0]] + (
                        item_old[1] - item_new[1]
                    )
    ...
    top_movers_filtered_sorted = dict(
        sorted(top_movers_filtered.items(), key=lambda item: item[1])
    )

    top_movers_filtered_sorted = dict(
        reversed(list(top_movers_filtered_sorted.items()))
    )
    ...
    if top_movers:
        for top_mover in top_movers:
            ...
            if AvgPrice > price_baseline[top_mover] * 1.02:
                print(
                    f"Trading pair:  {top_mover}, Price: {AvgPrice}, Price baseline: {price_baseline[top_mover]}, Rank: {top_movers[top_mover]}"
                )
